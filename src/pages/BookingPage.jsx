...
const BookingPage = () => {
  ...
  useEffect(() => {
    // Asegúrate de que `bookedSlots` sea un array
    fetch('https://api-party-kids.vercel.app/api/bookings')
      .then((response) => response.json())
      .then((data) => {
        const slots = Array.isArray(data) ? data : [];  // Asegúrate de que sea un array
    ...
    <div className="p-8 flex flex-col justify-center items-center bg-gradient-to-r from-violet-950 via-purple-600 to-blue-500 w-full min-h-screen font-robert-medium">
      ...
      <form onSubmit={handleSubmit} className="space-y-6 bg-white p-8 rounded-xl shadow-xl w-full sm:w-96">
        ...
        <div className="flex flex-col">
          ...
          <CalendarComponent onDateChange={handleDateChange} bookedSlots={bookedSlots} />
        </div>
