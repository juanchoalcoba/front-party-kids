...
const BookingPage = () => {
  ...
  const [bookedDates, setBookedDates] = useState([]); // Fechas reservadas desde el backend
  ...
  useEffect(() => {
    fetchBookedDates(); // Fetch initial bookings on mount
  }, []);
  ...
  const generateStartTimes = () => {
    ...
    const existingBooking = bookedDates.find(
      booking => booking.date.toDateString() === bookingData.date.toDateString()
    );

    if (existingBooking) {
      // Filtramos las horas ya reservadas
      const bookedStartTime = parseInt(existingBooking.timeSlot.split(':')[0], 10);

      if (bookingData.hours === '4') {
        while (startHour <= maxStartHourFor4Hours) {
          if (startHour < bookedStartTime || startHour >= bookedStartTime + 4) {
        ...
        while (startHour <= maxStartHourFor8Hours) {
          if (startHour < bookedStartTime || startHour >= bookedStartTime + 8) {
            times.push(`${startHour}:00`);
          }
          startHour++;
        }
      ...
      // Si no hay reservas para ese día, generamos las horas normalmente
      if (bookingData.hours === '4') {
        while (startHour <= maxStartHourFor4Hours) {
          times.push(`${startHour}:00`);
          startHour++;
