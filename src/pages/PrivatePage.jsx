...
const PrivatePage = () => {
  ...
  // Leer reserva con checkbox
  const handleViewedByAdmin = async (name) => {
    const confirm = window.confirm("¿Seguro quieres dejar leida esta reserva?");
    if (!confirm) return;
    ...
    try {
      ...
      alert("Reserva leida con éxito");
    } catch (error) {
      console.error("Error leyendo la reserva:", error);
      alert("Hubo un error al intentar leer la reserva");
    }
  };
              ...
              {pendingBookings.map((booking) => (
                ...
                >
                  ...
                  <td className="px-4 py-2 text-center">
                    <input
                      type="checkbox"
                      checked={booking.viewedByAdmin} // El estado del checkbox, basado en la reserva
                      onChange={() => handleViewedByAdmin(booking.name)} // Llamada a la función para cambiar el estado
                      disabled={booking.viewedByAdmin} // Deshabilitar si ya está marcado como true
