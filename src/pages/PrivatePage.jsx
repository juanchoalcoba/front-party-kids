...
const PrivatePage = () => {
  ...
  // Agrupar las reservas por mes y año, separando pendientes y confirmadas
  const groupBookingsByMonth = () => {
    ...
    bookings.forEach((booking) => {
      ...
      const group = booking.confirmed ? "confirmed" : "pending";

      if (!grouped[group][monthYear]) {
        grouped[group][monthYear] = [];
      }

      grouped[group][monthYear].push(booking);
    });
  ...
  return (
    <div className="bg-gray-900 min-h-screen p-6">
      {["pending", "confirmed"].map((status) => (
        <div key={status}>
          {Object.entries(groupedBookings[status]).map(([monthYear, bookings]) => (
            <div
              key={monthYear}
              className="max-w-6xl mx-auto bg-gray-800 shadow-xl rounded-lg p-8 mt-8 border-4 border-violet-400"
            >
              <h2 className="text-3xl font-bold text-center text-white mb-6">
                {status === "pending" ? "Reservas Pendientes" : "Reservas Confirmadas"} de {monthYear}
              </h2>
