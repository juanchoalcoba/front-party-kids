...
const PrivatePage = () => {
  ...
  const handleViewed = async (name) => {
    try {
      ...
      if (!response.ok) {
        const data = await response.json();
        throw new Error(data.message || "Error marcando la reserva como vista");
      }

      // Actualizar el estado para reflejar el cambio
      setBookings((prevBookings) =>
        prevBookings.map((booking) =>
          booking.name === name ? { ...booking, viewed: true } : booking
        )
      );
      alert("Reserva marcada como vista");
    } catch (error) {
      console.error("Error marcando la reserva como vista:", error);
      alert("Hubo un error al intentar marcar la reserva como vista");
    }
  };
              ...
              {pendingBookings.map((booking) => (
                ...
                >
                  ...
                  <td className="flex flex-row justify-center items-center px-4 py-2">
                    ...
                    <input
                      type="checkbox"
                      checked={booking.viewed || false} // Marcar si ya fue vista
